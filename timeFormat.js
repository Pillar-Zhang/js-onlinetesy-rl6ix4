const terminalTime = "1503190f2305000100C102a5";
import { isNaN } from "lodash";

const timeIsDate = obj => {
  const preIsDate = new Date(
    `20${obj.year}-${obj.month}-${obj.date} ${obj.hour}:${obj.minutes}:${
      obj.second
    }`
  ).getTime();
...
console.log(DateFormat(ConversionHexToTime(terminalTime)));
...
const headClockTimeFormat = str => {
  ...
  console.log(
    `${dateMap.year}-${dateMap.month}-${dateMap.date} ${dateMap.hour}:${
      dateMap.minutes
    }:${dateMap.second} ${dateMap.ms}.${dateMap.us}.${dateMap.ns}`
  );
  return `${dateMap.year}-${dateMap.month}-${dateMap.date} ${dateMap.hour}:${
    dateMap.minutes
  }:${dateMap.second} ${dateMap.ms}.${dateMap.us}.${dateMap.ns}`;
};

export { timeIsDate, DateFormat, ConversionHexToTime, headClockTimeFormat };
